...
<!-- wp:llms/lesson-navigation /-->
	...
	public function migrate_post() {
		...
		// TODO: Move to a new "class llms elementor migrate" class?
		// TODO: Move this to a method so that it can also be used to migrate existing content on LifterLMS plugin update.
		if ( $post && isset( $_REQUEST['action'] ) && 'elementor' === $_REQUEST['action'] && 'course' === get_post_type( $post->ID ) ) {
			if ( llms_parse_bool( get_post_meta( $post_id, '_llms_elementor_migrated', true ) ) ) {
				return;
			}
			...
			if ( $content ) {
				...
				update_post_meta( $post_id, '_llms_elementor_migrated', 'yes' );
			}
			return;
		}
